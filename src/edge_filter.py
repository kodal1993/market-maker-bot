...
class EdgeFilter:
    ...
    def assess(
        ...
        *,
        ...
        elif expected_edge_usd <= 0:
            edge_reject_reason = "expected_edge_negative"
        elif expected_edge_bps < required_edge_bps or expected_edge_usd < required_edge_usd:
        ...
        elif edge_score < required_edge_score:
        ...
        if (
            consecutive_losses >= MAX_CONSECUTIVE_LOSSES_BEFORE_PAUSE
            and not is_protective_exit(side, signal.reason)
            and edge_score < HIGH_EDGE_OVERRIDE_SCORE
            and not edge_reject_reason
        ):
            edge_reject_reason = "losing_streak_pause"
